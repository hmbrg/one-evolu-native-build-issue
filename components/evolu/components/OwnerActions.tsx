...
import { useEvolu, useOwner, NonEmptyString1000 } from "@/lib/evolu/evolu";
...
export const OwnerActions: FC = () => {
  ...
  const handleMnemonicInputEndEditing = () => {
    console.log("handleMnemonicInputEndEditing", parsedMnemonic);
    Either.match(parsedMnemonic, {
      onLeft: (error) => alert(formatError(error)),
      onRight: (nmnemonic) => evolu.restoreOwner(nmnemonic, { reload: true }),
    });
  };
